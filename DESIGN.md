...
| `hot_path_profile_balance` | Same unpaused scene as `hot_path_profile` (six stoves, six colonists, 900 frames, 0 paused), in Balance + "one of each first" with every bill on vanilla's slow map-wide count. The urgency prefix: **max 0.048 ms in any frame**, 1.21 µs per call, 0.0003 ms/frame on average. The same prefix in a round-robin group is 0.14 µs per call and max 0.006 ms, the cost of the early-out. |

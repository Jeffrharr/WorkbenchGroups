...
| `hot_path_profile_balance` | `hot_path_profile`'s scene (six linked stoves, six colonists, 900 unpaused frames), now actually powered and set to mid-morning, with the group in Balance plus "one of each first" and every bill on vanilla's slow map-wide count. Gated on the colony walking its bills (`ShouldDoNow` called at least 20 times) and all six stoves powered. The urgency prefix: **worst frame 0.26 ms** (1.5% of a 60 fps frame; the frame where 42 scans landed at once), 0.0003 ms/frame on average, 6.9 µs per call. In a round-robin group it early-outs at 0.43 µs per call, worst frame 0.012 ms. Routing the overshoot guard's count through the same cache took that postfix's worst frame in a Balance group from 0.41 ms to 0.007 ms. |
